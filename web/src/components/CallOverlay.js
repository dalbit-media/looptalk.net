...
const CALL_STATUS_CHANNEL = "looptalk-call-status";
const INCOMING_CALL_STORAGE_KEY = "looptalk:incoming-call";
...
export const CallOverlay = () => {
  ...
  useEffect(() => {
    ...
    const ringing = call?.status === "ringing" && call.direction === "incoming";
    const message = {
      type: "incoming-call",
      ringing,
    ...
    try {
      globalThis.localStorage.setItem(INCOMING_CALL_STORAGE_KEY, JSON.stringify(message));
    } catch {}
    const channel =
      typeof globalThis.BroadcastChannel === "function"
        ? new globalThis.BroadcastChannel(CALL_STATUS_CHANNEL)
        : null;
    channel?.postMessage(message);
    return () => {
      if (ringing) {
        const endedMessage = { ...message, ringing: false, timestamp: Date.now() };
        try {
          globalThis.localStorage.setItem(
            INCOMING_CALL_STORAGE_KEY,
            JSON.stringify(endedMessage)
          );
        } catch {}
        channel?.postMessage(endedMessage);
      }
      channel?.close();
    };
  }, [call?.callId, call?.status, call?.direction]);
